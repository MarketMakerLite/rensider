...
export default function InsidersError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('Insiders page error:', error)
  }, [error])

  return (
    <ApplicationLayout>
      <div className="max-w-2xl py-16 text-center">
        <Heading>Failed to load insider transactions</Heading>
        <Text className="mt-2 text-zinc-600">
          Insider transaction data could not be loaded. Please try again.
        </Text>
        {error.digest && (

...
export default function NewFilingsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('New filings page error:', error)
  }, [error])

  return (
    <ApplicationLayout>
      <div className="max-w-2xl py-16 text-center">
        <Heading>Failed to load latest filings</Heading>
        <Text className="mt-2 text-zinc-600">
          Recent filing data could not be loaded. Please try again.
        </Text>
        {error.digest && (
